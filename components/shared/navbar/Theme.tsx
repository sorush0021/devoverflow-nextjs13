...
import { Menubar, MenubarContent, MenubarItem, MenubarMenu,  MenubarTrigger,
...
const Theme = () => {
  ...
  return (
    ...
    <MenubarMenu>
      <MenubarTrigger className='focus:bf-light-900 data-[state=open]:bg-light-900
        ...
        {themes.map((item)=>(
            ...
            className='dark:focus:bg-dark-400 flex items-center gap-4 px-2.5 py-2'>
                ...
                <p className={`body-semibold text-light-500 ${mode===item.value ? 'text-primary-500':
                    'text-dark-100_light900'}`}>{item.label}</p>
            </MenubarItem>
        ))}
      </MenubarContent>
    </MenubarMenu>
